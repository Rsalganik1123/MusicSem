...
class EvaluationHelperParallel:
    ...
    def init_models(self, rank):
        ...
        if self.backbone == "mert":
            ...
            self.mel_model = AutoModel.from_pretrained("m-a-p/MERT-v1-95M", config=config, trust_remote_code=True)
    ...
    #         )
    #         return False
    def get_filename_intersection_ratio(self, dir1, dir2, *_, **__):
        """
        Compare the contents of two directories and **return True un‑conditionally**.
        Any file that appears in just one of the locations is announced and will be
        ignored later by the per‑metric routines (e.g. our patched `calculate_kl`).

        Parameters
        ----------
        dir1 : str  – generation directory
        dir2 : str  – reference directory
        *_, **__    – extra positional / keyword args are accepted and ignored
                      (so existing call‑sites do not need to change)
        """
        assert os.path.exists(dir1), f"Path does not exist: {dir1}"
        assert os.path.exists(dir2), f"Path does not exist: {dir2}"

        files_gen = {os.path.basename(p) for p in os.listdir(dir1)}
        files_ref = {os.path.basename(p) for p in os.listdir(dir2)}

        common      = files_gen & files_ref
        only_gen    = files_gen - files_ref
        only_ref    = files_ref - files_gen

        print(f"⇒ {len(common)} common file(s) will be evaluated.")
        if only_gen:
            print(f"• Skipping {len(only_gen)} generation‑only file(s).")
        if only_ref:
            print(f"• Skipping {len(only_ref)} reference‑only file(s).")

        # Always treat the two folders as “same‑name” now – the metric
        # functions themselves are responsible for dropping unmatched files.
        return True
